...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON teams.team_id = winner_id OR teams.team_id = opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(name) FROM games LEFT JOIN teams ON teams.team_id = winner_id ORDER BY name")"
